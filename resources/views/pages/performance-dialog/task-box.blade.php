@extends('layouts_.vertical', ['page_title' => 'Task Box'])
...
@section('content')
    <div class="container-fluid">
        ...
        <form id="form-filter-task-box" action="{{ route('performance-dialog-task') }}" method="GET">
            <div class="row align-items-end">
                <div class="col-auto">
                    <div class="mb-3">
                        <label class="form-label" for="period">{{ __('Year') }}</label>
                        <select name="period" id="period" onchange="filterTaskBoxYears(this.value)" class="form-select border-secondary" style="width: 180px">
        ...
        @forelse ($performance_dialogs as $index => $row)
            <div class="row">
                <div class="col-md-12">
                <div class="card shadow p-0 task-box-card" data-year="{{ $row->period }}">
        ...
        @empty
            <div class="row">
                ...
                <div class="card shadow mb-4">
                    <div class="card-body">
                        {{ __('No Task Box Found ') }}<i class="ri-arrow-right-up-line"></i>
                    </div>
                </div>
                </div>
            </div>
...
@push('scripts')
    ...
    <script>
        ...
        function filterTaskBoxYears(year) {
            const cards = document.querySelectorAll('.task-box-card');
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (sel) filterTaskBoxYears(sel.value);
        });

        window.filterTaskBoxYears = filterTaskBoxYears;
    </script>
@endpush
