                    ...
                    <div class="card-body">
                        ...
                        <div class="row px-2">
                            ...
                            <div class="col-lg col-sm-12 p-2">
                                <h5>Status</h5>
                                <div>
                                    <a href="javascript:void(0)" data-bs-id="{{ $row->request->employee_id }}" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="{{ $row->request->goal->form_status == 'Draft' ? 'Draft' : ($row->approvalLayer && $row->request->goal->form_status == 'Approved' ? 'Manager L'.$row->approvalLayer.' : '.$row->name : 'Manager L'.$row->approvalLayer.' (Goals were auto-approved after you submitted PA '.$row->request->period .')') }}" class="badge {{ $row->request->goal->form_status == 'Draft' || $row->request->sendback_to == $row->request->employee_id ? 'bg-secondary' : ($row->request->appraisalCheck ? ($row->request->status === 'Approved' ? 'bg-success' : 'text-bg-light' ) : 'bg-warning')}} rounded-pill py-1 px-2">
