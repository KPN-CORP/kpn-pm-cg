@forelse ($team->contributors as $contributor)
    ...
    {{-- @if (strtolower($contributor->status) == 'approved')
        <a href="{{ route('performance-dialog.form', ["contributor_id" => encrypt($contributor->id), "period" => $contributor->period]) }}" title="{{ __('Performance Review') }}" type="button" class="btn btn-outline-warning btn-sm m-1"><i class="ri-clipboard-line"></i></a>
    @endif --}}
@empty
